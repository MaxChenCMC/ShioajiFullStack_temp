...
const NavBar = () => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                // const response = await fetch('http://localhost:57064/api/ListAccountsSnapshots_NavBar');
    ...
    // const [TbarTseTxfOtc, setTbarTseTxfOtc] = useState<Data>({ Acct: [], TAIFEX: [], TSE: [], OTC: [] });
    // useEffect(() => {
    //   axios.get('http://localhost:9033/api/ListAccountsSnapshots_NavBar')
    //     .then(response => setTbarTseTxfOtc(response.data))
    //     .catch(error => console.log(error));
    // }, []);    
